...
"""Amiga Bridegboard autoboot harddisk file binary image converter.
   ...
   to a Amiga virtial PC disk HDD image usable from a Commodore Bridgeboard and
   vice versa)

   Written by Heiko Prüssing in 2021,2022
"""

import sys, struct, fileinput, os, math
    ...
    while True:
        try:
            ...
            if val < min or val > max:
                print("Error, the number must be in range [%d..%d]" % (min,max) )
            else:
                return val
        except ValueError:
...
def main(argv):
  ...
  if abootHeader[0] == b"ABOOT\0\0\0":
      ...
      #ch = input("Should the file be converted to a Amiga PC virtual HDD image (usable as hard disk for a Amiga Bridgeboard) [y/n]? ")
      #if ch == 'y':
      if True:
          ...
          # Try to detecting the CHS geometry from the MBR in the file
          # detectExistingCHSGeometryFromMBR(filename)
          ...
          fout.close()
          fin.close()
          print("Success. Now you can use '%s' as a autoboot harddisk file for your Amiga Bridgeboard. Have fun ;)" % (ofilestring) )
          exit()
